...
def SMB(SMB_inputs):
    ...
    print(f'kav_params_all: {kav_params_all}')
    print(f'D_all: {D_all}')
    print(f'ncomp: {num_comp}')
...
def see_prod_curves(t_odes, Y, t_index) :
    ...
    for i in range(num_comp): # for each component
        if iso_type == "UNC":
            ax[0].plot(t_odes[i], Y[0][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
            ax[1].plot(t_odes[i], Y[1][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
            ax[2].plot(t_odes[i], Y[2][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
        
        elif iso_type == "CUP":    
            ax[0].plot(t_odes, Y[0][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
            ax[1].plot(t_odes, Y[1][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
            ax[2].plot(t_odes, Y[2][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
    ...
    for i in range(num_comp): # for each component
        if iso_type == "UNC":
            ...
            vx[0].plot(t_odes[i], Y[3][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
            vx[1].plot(t_odes[i], Y[4][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
        
        elif iso_type == "CUP":    
            
            vx[0].plot(t_odes, Y[3][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
            vx[1].plot(t_odes, Y[4][i], color = color[i], label = f"{Names[i]}, {Names[i]}:{cusotom_isotherm_params_all[i]}, kh:{kav_params_all[i]}")
...
def coupled_animate_profiles(t, title, y, nx, labels, colors, t_start_inject_all, t_index, L_total, parameter_sets, Ncol_num):
    def create_animation(y_profiles, t, concentration_type, filename, labels, colors,L_total,  parameter_sets, Ncol_num):
        ...
        for i, y_profile in enumerate(y_profiles):
            line, = ax.plot(x, y_profile[:, 0], label=f"{labels[i]}: H{labels[i]} = {cusotom_isotherm_params_all[i]}, kh{labels[i]} = {kav_params_all[i]}", color=colors[i])
            lines.append(line)
...
parameter_sets = [
                    {"C_feed": 0.0022},    # Borate g/cm^3
                    {"C_feed": 0.0022}] #, # HCl g/cm^3
