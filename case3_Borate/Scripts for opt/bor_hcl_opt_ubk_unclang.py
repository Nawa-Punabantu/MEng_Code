...
def SMB(SMB_inputs):
    ...
    def mass_transfer(kav_params, q_star, q ): # already for specific comp
        ...
        # 2. Two Parameter Model - Combine PFO and PSO
        # Unpack Parameters
        kav1 =  kav_params[0]
        kav2 =  kav_params[1]
    ...
    def cusotom_CUP_isotherm_func(cusotom_isotherm_params_all, c, IDX, comp_idx):
        ...
        *****
        ...
        Qmax_i =  cusotom_isotherm_params_all[comp_idx][0]
        b_i =  cusotom_isotherm_params_all[comp_idx][1]
        
        # Fix the sum of parameters in the demoninator:
        b1 = cusotom_isotherm_params_all[0][1] # 1st (and only) parameter of HA 
        b2 = cusotom_isotherm_params_all[1][1] # 1st (and only) parameter of HB
...
kav_params_all = np.array([[1.8647, 1.8647], [1.683276, 1.8647]])
...
#%%
    ...
    with open("UBK-borhcl-unclang-type1_60iter_all_inputs.json", "w") as f:
        json.dump(all_inputs_list, f, indent=4)

    # SAVE recoveries_and_purities to JSON:
    with open("UBK-borhcl-unclang-type1_60iter_all_outputs.json", "w") as f:
