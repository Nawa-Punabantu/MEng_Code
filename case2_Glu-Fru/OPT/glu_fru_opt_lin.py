...
################ Time Specs #################################################################################
t_index_min = 6.67 # min # Index time # How long the pulse holds before swtiching
...
parameter_sets = [
                    {"C_feed": 0.42},    # Borate g/cm^3
                    {"C_feed": 0.42}] #, # HCl g/cm^3
...
SMB_inputs = [iso_type, Names, color, num_comp, nx_per_col, e, Da_all, Bm, zone_config, L, d_col, d_in, t_index_min, n_num_cycles, Q_internal, parameter_sets, cusotom_isotherm_params_all, kav_params_all, subzone_set]
...
def plot_raff_ext_profiles(time_sim, raff_cprofile, ext_cprofile,
                           time_exp=None, raff_exp_data=None, ext_exp_data=None):
    """
    Plot raffinate and extract concentration profiles over time.
    
    Parameters:
    - time_sim: array-like, time vector from simulation (seconds)
    - raff_cprofile: list of arrays for simulated raffinate concentrations
    - ext_cprofile: list of arrays for simulated extract concentrations
    - time_exp: optional array-like, time vector for experimental data (seconds)
    - raff_exp_data: optional list of arrays for experimental raffinate data
    - ext_exp_data: optional list of arrays for experimental extract data
    """
    time_sim_hrs = time_sim / 3600
    time_exp_hrs = time_exp / 3600 if time_exp is not None else None

    fig, ax = plt.subplots(1, 2, figsize=(15, 5))

    # Raffinate subplot
    for i, curve in enumerate(raff_cprofile):
        ax[0].plot(time_sim_hrs, curve, label=f'Raff Sim {i}')
    if time_exp_hrs is not None and raff_exp_data:
        for i, curve in enumerate(raff_exp_data):
            ax[0].plot(time_exp_hrs, curve, '--', label=f'Raff Exp {i}')

    ax[0].set_title('Raffinate Profiles')
    ax[0].set_xlabel('Time [hr]')
    ax[0].set_ylabel('Concentration [g/mL]')
    ax[0].legend()
    ax[0].grid(True)

    # Extract subplot
    for i, curve in enumerate(ext_cprofile):
        ax[1].plot(time_sim_hrs, curve, label=f'Extract Sim {i}')
    if time_exp_hrs is not None and ext_exp_data:
        for i, curve in enumerate(ext_exp_data):
            ax[1].plot(time_exp_hrs, curve, '--', label=f'Extract Exp {i}')

    ax[1].set_title('Extract Profiles')
    ax[1].set_xlabel('Time [hr]')
    ax[1].set_ylabel('Concentration [g/mL]')
    ax[1].legend()
    ax[1].grid(True)
...
# Display the plot
sim_time = results[2]
raff_sim = raff_cprofile
ext_sim = ext_cprofile # g/cm^3

exp_time = np.array([0, 1800, 3600, 5400, 7200])  # in seconds, for example
raff_exp = [np.array([0.1, 0.12, 0.15, 0.18, 0.2]),
            np.array([0.05, 0.07, 0.09, 0.11, 0.13])]
ext_exp = [np.array([0.3, 0.28, 0.26, 0.25, 0.23]),
           np.array([0.2, 0.19, 0.17, 0.15, 0.13])]

# Call the plotting function
plot_raff_ext_profiles(sim_time, raff_sim, ext_sim,
                       time_exp=exp_time,
                       raff_exp_data=raff_exp,
                       ext_exp_data=ext_exp)
...
#%%
    ...
    Description = [f"Description example: Optimizting the Borate HCl system for the sythetic solution. {optimization_budget} iterations, with no isotherom data. We placed a upper flowrate constraint of {Q_max} L/h. This is with the sub-zoning for bor-hcl"]
    ...
    with open("UBK-borhcl-type1_150iter_all_inputs.json", "w") as f:
        json.dump(all_inputs_list, f, indent=4)

    # SAVE recoveries_and_purities to JSON:
    with open("UBK-borhcl-type1_150iter_all_outputs.json", "w") as f:
