...
from Col_test_model_func_gf import column_func
...
def objective_function(params, t_data, conc_data, column_func_inputs, max_of_each_input, counter):
    """
    Func that: 
    ...
    counter => 0 if solving for 1st comp (glucose) and 1 if solving for 2nd component (fructose)
    """
    ...
    # ----------------------------------------------------------------------    
    cusotom_isotherm_params = cusotom_isotherm_params_all[counter]
    # ----------------------------------------------------------------------    

    # If not fixing the isotherm parameters:
...
def get_data_from_excel(file_path, t_start, t_end, resolution) :
    ...
    # Fred:
    cusotom_isotherm_params_all = np.array([[1.70, 1.19],[0.056, 1.81]])
    # Langmuir:
    # cusotom_isotherm_params_all = np.array([[0.500, 9.14],[0.091, 9.26]])    
...
def constrained_BO(optimization_budget, bounds, all_initial_inputs, all_initial_ouputs, t_data, conc_data, column_func_inputs, max_of_each_input, counter ,xi, sampling_budget=1):
    """
    ...
    for gen in range(optimization_budget):
        ...
        f_new, __ = objective_function(x_new, t_data, conc_data, column_func_inputs, max_of_each_input, counter)
...
if __name__ == "__main__":
    ...
    # Da guesses:
    Da_bor_guess = 0.00351 # cm^2/s
    Da_hcl_guess = 0.001174 # cm^2/s
    # kfp guesses:
    kfp_bor_guess = 0.056
    kfp_hcl_guess = 0.0001
    # Isotherm Guesses
    K1_bor_guess = 4.60
    K1_hcl_guess = 0.742 
    #//
    ...
    # --------- 3. BORATE & HCL DATA
    # UBK:
    file_path_glucose = r"C:\Users\28820169\Downloads\BO_Papers\MEng_Code\case2_Glu-Fru\_glucose.xlsx"
    file_path_fructose = r"C:\Users\28820169\Downloads\BO_Papers\MEng_Code\case2_Glu-Fru\_fructose.xlsx"

    SMB_resin_comps = [file_path_glucose, file_path_fructose, 'SMB-Resin']
    ...
    # Step 1:
    resin_select = SMB_resin_comps[-1] 

    # Step 2: Use # SMB_resin_comps[:-1]
    counter = 0
    for comp in SMB_resin_comps[:-1]: 
            ...
            f_initial, elution_curve_initial_guess = objective_function(x_initial_guess, t_data, col_elution_data, column_func_inputs,max_of_each_input, counter)
            ...
            f_vals, norm_all_inputs = constrained_BO(optimization_budget, bounds, x_initial_guess, f_initial, t_data, col_elution_data, column_func_inputs, max_of_each_input,counter, xi=0.005, sampling_budget=1)
            # print(f'all_inputs !!!!!!!!: \n{ all_inputs}')
            ...
            __, elution_curve_best = objective_function(norm_best_inputs, t_data, col_elution_data, column_func_inputs,max_of_each_input, counter)
